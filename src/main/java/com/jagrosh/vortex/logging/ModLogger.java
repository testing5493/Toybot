...
public class ModLogger {
    ...
    private synchronized void filterEntries(List<AuditLogEntry> entries) {
        ...
        try { //TODO: Figure out why I even put first run and why is it tied to developer mode
    ...
    private void update(Guild guild) // not async
    {
        ...
        try {
            ...
            for (AuditLogEntry ale : vortex.getDatabase().auditcache.filterUncheckedEntries(list)) {
                Action act = null;
                switch (ale.getType()) {
                    case BAN -> act = Action.BAN;
                    case KICK -> act = Action.KICK;
                    case UNBAN -> act = Action.UNBAN;
