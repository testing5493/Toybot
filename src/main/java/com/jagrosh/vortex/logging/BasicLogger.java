...
public class BasicLogger {
    ...
    public void logMessageDelete(CachedMessage oldMessage) {
        ...
        log(guild, embedBuilder -> embedBuilder.setColor(Color.yellow).setAuthor(author == null ? getLoggingName(oldMessage) : getLoggingName(guild, author), null, author == null ? null : author.getEffectiveAvatarUrl()).appendDescription("**Message sent by <@" + (author == null ? oldMessage.getAuthorId() : author.getId()) + "> deleted in <#" + oldMessage.getTextChannelId() + ">**\n").appendDescription(formatted).setFooter("Author ID: " + (author == null ? oldMessage.getAuthorId() : author.getId()) + " | Message ID: " + oldMessage.getId(), null).setTimestamp(Instant.now()));
    }
    ...
    public void logMessageBulkDelete(List<CachedMessage> messages, int count, TextChannel text) {
        ...
        vortex.getTextUploader().upload(LogUtil.logCachedMessagesForwards("Deleted Messages", messages, vortex.getJda()), "DeletedMessages", (view, download) -> log(guild, embedBuilder -> embedBuilder.setColor(Color.YELLOW).setDescription(String.format("**Bulk delete in %s, %d messages were deleted**\nClick to [view](%s) or [download](%s) the deleted messages.", text.getAsMention(), count, view, download)).setFooter("Channel ID: " + text.getId(), null).setTimestamp(Instant.now())));
    }
    ...
    public void postCleanCase(Member moderator, OffsetDateTime now, int numMessages, TextChannel target, String criteria, String reason, String view, String download) {
        log(target.getGuild(), embedBuilder -> embedBuilder.setColor(Color.YELLOW).setAuthor(getLoggingName(moderator.getGuild(), moderator.getUser()), null, moderator.getUser().getEffectiveAvatarUrl()).appendDescription(String.format("**%s purged %d messages in %s**\n**Criteria:** %s\n%sClick to [view](%s) or [download](%s) the deleted messages.", moderator.getAsMention(), numMessages, target.getAsMention(), criteria, reason == null || reason.trim().isEmpty() ? "" : "**Reason:** " + reason + "\n", view, download)).setFooter("Mod ID: " + moderator.getUser().getId() + " | Channel ID: " + target.getId(), null).setTimestamp(now));
    }
    ...
    public void logNameChange(UserUpdateNameEvent event) {
        ...
        user.getMutualGuilds().stream().map(guild -> vortex.getDatabase().settings.getSettings(guild).getServerLogChannel(guild)).filter(Objects::nonNull).forEachOrdered(tc -> {
            log(tc.getGuild(), embedBuilder -> embedBuilder.setAuthor(getLoggingName(tc.getGuild(), user), null, user.getEffectiveAvatarUrl()).setColor(Color.GREEN).setDescription(String.format("**%s has changed their username from %s#%s to %s#%s**", user.getAsMention(), event.getOldName(), user.getDiscriminator(), event.getNewName(), user.getDiscriminator())).setFooter("User ID: " + event.getUser().getId(), null).setTimestamp(now));
        });
    }
    ...
    public void logNameChange(UserUpdateDiscriminatorEvent event) {
        ...
        event.getUser().getMutualGuilds().stream().map(guild -> vortex.getDatabase().settings.getSettings(guild).getServerLogChannel(guild)).filter(Objects::nonNull).forEachOrdered(tc -> {
            log(tc.getGuild(), embedBuilder -> embedBuilder.setAuthor(getLoggingName(tc.getGuild(), user), null, user.getEffectiveAvatarUrl()).setColor(Color.GREEN).setDescription(String.format("**%s has changed their username from %s#%s to %s#%s**", user.getAsMention(), user.getName(), event.getOldDiscriminator(), user.getName(), event.getNewDiscriminator())).setFooter("User ID: " + event.getUser().getId(), null).setTimestamp(now));
        });
    }
    ...
    public void logGuildJoin(GuildMemberJoinEvent event, OffsetDateTime now) {
        ...
        if (event.getUser().getTimeCreated().isAfter(now)) {
            ...
            newText = String.format("Account created on %s (%s ago)", event.getUser().getTimeCreated().format(DateTimeFormatter.RFC_1123_DATE_TIME), FormatUtil.secondsToTimeCompact(seconds));
        }

        Guild guild = event.getGuild();
        User user = event.getUser();
        log(guild, embedBuilder -> embedBuilder.setAuthor(getLoggingName(guild, user), null, user.getEffectiveAvatarUrl()).setThumbnail(user.getEffectiveAvatarUrl()).setColor(Color.BLUE).appendDescription("**" + user.getAsMention() + " joined the server**\n").appendDescription(newText).setFooter("User ID: " + user.getId(), null).setTimestamp(now));
    }
    ...
    public void logGuildLeave(GuildMemberRemoveEvent event) {
        ...
        log(guild, embedBuilder -> embedBuilder.setAuthor(getLoggingName(guild, user), null, user.getEffectiveAvatarUrl()).setThumbnail(user.getEffectiveAvatarUrl()).setColor(Color.BLUE).appendDescription("**" + user.getAsMention() + " left or was kicked/banned from the server**").appendDescription(String.format("\nThis user originally joined on %s (%s ago)", member.getTimeJoined().format(DateTimeFormatter.RFC_1123_DATE_TIME), FormatUtil.secondsToTimeCompact(seconds))).appendDescription(rlist.toString().isEmpty() ? "" : rlist.toString()).setFooter("User ID: " + user.getId(), null).setTimestamp(now));
    }
    ...
    private void logVoiceJoin(GuildVoiceUpdateEvent event) {
        ...
        log(guild, embedBuilder -> embedBuilder.setAuthor(getLoggingName(guild, user), null, user.getEffectiveAvatarUrl()).setColor(Color.BLUE).appendDescription(String.format("**%s joined the voice channel <#%s>**", user.getAsMention(), vcId)).setFooter("User ID: " + user.getId() + " |  VC ID: " + vcId, null).setTimestamp(OffsetDateTime.now()));
    }
    ...
    private void logVoiceMove(GuildVoiceUpdateEvent event) {
        ...
        User user = member.getUser();
        log(guild, embedBuilder -> embedBuilder.setAuthor(getLoggingName(guild, user), null, user.getEffectiveAvatarUrl()).setColor(Color.BLUE).appendDescription(String.format("**%s moved voice channels from <#%s> to <#%s>**", user.getAsMention(), oldVcId, newVcId)).setFooter(String.format("User ID: %s | Old VC Id: %s | New VC ID: %s", user.getId(), oldVcId, newVcId), null).setTimestamp(OffsetDateTime.now()));
    }
    ...
    private void logVoiceLeave(GuildVoiceUpdateEvent event) {
        ...
        User user = member.getUser();
        log(guild, embedBuilder -> embedBuilder.setAuthor(getLoggingName(guild, user), null, user.getEffectiveAvatarUrl()).setColor(Color.BLUE).appendDescription(String.format("**%s left the voice channel <#%s>**", user.getAsMention(), vcId)).setFooter("User ID: " + user.getId() + " |  VC ID: " + vcId, null).setTimestamp(OffsetDateTime.now()));
    }
    ...
    public void logModlog(Guild guild, Modlog modlog) {
        ...
        log(guild, embedBuilder -> embedBuilder.setAuthor(getLoggingName(guild, user), null, user.getEffectiveAvatarUrl()).setColor(Color.RED).appendDescription(String.format("<@%d> %s <@%d>%s", punishing ? modlog.getModId() : modlog.getSaviorId(), punishing ? verb : "un" + verb, modlog.getUserId(), HOW_LONG)).setFooter(String.format("User ID: %d | %s ID: %d", modlog.getUserId(), punishing ? "Mod" : "Savior", punishing ? modlog.getModId() : modlog.getSaviorId()), null).appendDescription("\n**Case ID:**" + modlog.getId()).appendDescription(modlog.getReason() == null || modlog.getReason().trim().isEmpty() ? "" : "\n**Reason:**" + modlog.getReason()).setTimestamp(punishing ? modlog.getStart() : modlog.getFinnish()));
    }
    ...
    public void logModlogUpdate(Guild guild, long caseId, User updatingModerator, String oldReason, String newReason, Temporal now) {
        log(guild, embedBuilder -> embedBuilder.setAuthor(getLoggingName(guild, updatingModerator), null, updatingModerator.getEffectiveAvatarUrl()).setColor(Color.ORANGE.darker()).appendDescription(updatingModerator.getAsMention() + " updated the reason for case " + caseId).addField("Old Reason", oldReason.isEmpty() ? "_No Reason Specified_" : oldReason, false).addField("New Reason", newReason, false).setFooter("Updating Moderator ID: " + updatingModerator.getId(), null).setTimestamp(now));
    }

    public void logModlogDeletion(Guild guild, Modlog modlog, User deletingModerator) {
        log(guild, embedBuilder -> embedBuilder.setAuthor(getLoggingName(guild, deletingModerator), null, deletingModerator.getEffectiveAvatarUrl()).setColor(Color.ORANGE.darker()).appendDescription(deletingModerator.getAsMention() + " deleted a modlog").addField("Case " + modlog.getId(), FormatUtil.formatModlogCase(vortex, guild, modlog), true).setFooter(String.format("Deleting Moderator ID: %s | User ID: %d | Punishing Moderator ID:%d%s", deletingModerator.getId(), modlog.getUserId(), modlog.getModId(), modlog.getSaviorId() == -1 ? "" : " | Savior ID: " + modlog.getSaviorId()), null).setTimestamp(Instant.now()));
    }
