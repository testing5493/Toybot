...
public class VoicelogCmd extends LogCommand {
    public VoicelogCmd(Vortex vortex) {
        super(vortex);
        this.name = "voicelog";
    ...
    protected void showCurrentChannel(CommandEvent event) {
        ...
        if (tc == null) {
            ...
            event.replySuccess("Voice Logs are currently being sent in " + tc.getAsMention() + (event.getSelfMember().hasPermission(tc, REQUIRED_PERMS) ? "" : "\n" + event.getClient().getWarning() + String.format(REQUIRED_ERROR, tc.getAsMention())));
        }
    }
