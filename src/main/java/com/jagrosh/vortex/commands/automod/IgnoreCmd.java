    ...
    protected void execute(CommandEvent event) {
        if (event.getArgs().isEmpty()) {
            ...
            event.getGuild().getRoles().stream().forEach(r -> {
                ...
                } else if (r.getPermissions().contains(Permission.ADMINISTRATOR) || r.getPermissions().contains(Permission.MANAGE_SERVER) || r.getPermissions().contains(Permission.BAN_MEMBERS) || r.getPermissions().contains(Permission.KICK_MEMBERS) || r.getPermissions().contains(Permission.MESSAGE_MANAGE)) {
