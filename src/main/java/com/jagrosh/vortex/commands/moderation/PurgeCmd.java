    ...
    protected void execute(CommandEvent event) {
        ...
        event.async(() -> {
            ...
            for (Message msg : msgs) {
                ...
                if (all || ids.contains(msg.getAuthor().getId()) || (bots && msg.getAuthor().isBot()) || (embeds && !msg.getEmbeds().isEmpty()) || (links && LINK_PATTERN.matcher(msg.getContentRaw()).find()) || (images && hasImage(msg)) || (mentions && MENTION_PATTERN.matcher(msg.getContentRaw()).find())) {
                    del.add(msg);
                    continue;
                }
            ...
            vortex.getTextUploader().upload(LogUtil.logMessagesBackwards("Cleaned Messages", del), "CleanedMessages", (view, download) -> {
                vortex.getBasicLogger().postCleanCase(event.getMember(), event.getMessage().getTimeCreated(), del.size(), event.getTextChannel(), params, null, view, download);
            });
        });
    }
