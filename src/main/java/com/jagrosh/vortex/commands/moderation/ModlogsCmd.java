...
public class ModlogsCmd extends ModCommand {
    ...
    protected void execute(CommandEvent event) {
        ...
        if (event.getJDA().getUserById(id) != null) {
            embeds[0].setAuthor(String.format("%d modlog%s found for %s#%s (%d)", size, size == 1 ? "" : "s", event.getJDA().getUserById(id).getName(), event.getJDA().getUserById(id).getDiscriminator(), id), null, String.format(event.getJDA().getUserById(id).getAvatarUrl()));
        } else {
            embeds[0].setAuthor(String.format("%d modlog%s found for %d", size, size == 1 ? "" : "s", id), null, String.format(event.getJDA().getUserById(id).getAvatarUrl()));
        }
